...
#cureve fit-----------------------------------------------
def linear_line(x1, x2, y1, y2, sub_num):
	m = (y2 - y1)/(x2-x1)  
	n = y1 - (m * x1) 
	f_y = []
	x_slice = np.linspace(x1, x2, sub_num)
  	for i in x_slice:
		fx = m * i + n
		f_y.append(fx)
		fy = np.array(f_y)
...
def funcx3 (X, a, b, c, d):
    return a*X + b*X**2 + c*X**3 + d
	

def curve_fitting(arr, sub_num): #arr : sorted yellow_arr or blue_arr [0] : flag / [1] : x  / [2] : y  
    sub_x = np.array([])
    sub_y = np.array([])
    
    for i in range(len(arr)-1):
        x_lst = (np.linspace((arr[i][1]), (arr[i+1][1]), sub_num))  
        sub_x = np.concatenate([sub_x, x_lst])
        linear_y = linear_line(arr[i][1], arr[i+1][1], arr[i][2], arr[i+1][2], sub_num)
        sub_y = np.concatenate([sub_y, linear_y])
    
    popt, pcov = curve_fit(funcx1, sub_x, sub_y)
    sub_y = funcx1(sub_x, *popt)
    return sub_x, sub_y


def curve_fitting_2point(x1, x2, y1, y2, sub_num): 
	sub_x = (np.linspace(x1, x2, sub_num))
	sub_y = linear_line(x1,x2,y1,y2,sub_num)
	popt, pcov = curve_fit(funcx2, sub_x, sub_y)
	sub_y = funcx2(sub_x, *popt)
	return sub_x, sub_y
...
if __name__ == '__main__':
	...
	while not rospy.is_shutdown(): #cap.isOpened()
		...
		if (len(new_data_list) > 0):
			...
			yellow_arr.append([warp_left_point[2],warp_left_point[0],warp_left_point[1]])
			blue_arr.append([warp_right_point[2],warp_right_point[0],warp_right_point[1]])
			#x sort
			#yellow_arr_curve = sorted(yellow_arr, key=lambda x:x[1])
			#blue_arr_curve = sorted(blue_arr, key=lambda x:x[1])
			...
			if (len(yellow_arr) == 2): # 2 -> only one cone
			...
			if (len(yellow_arr) >= 3): 
				...
				x, y = curve_fitting(yellow_arr, 20)
				for k in range((len(x)-2), 0 , -1): 	
					cv2.line(yellow_img, (int(x[k]),int(y[k])), (int(x[k+1]),int(y[k+1])), (255,255,255), 15)  #curvefitting only yello cone
					cv2.line(img_transformed_y, (int(x[k]),int(y[k])), (int(x[k+1]),int(y[k+1])), (255,0,0), 15)  #curvefitting only yello cone


				'''
				#modify-----------------------------------------------------------------------------
				x,y = curve_fitting(yellow_arr_curve,20) 
				for k in range(len(x)-1): 
					cv2.line(yellow_img, (int(x[k]),int(y[k])), (int(x[k+1]),int(y[k+1])), (255,255,255), 15)  #curvefitting only yello cone
					cv2.line(img_transformed_y, (int(x[k]),int(y[k])), (int(x[k+1]),int(y[k+1])), (255,0,0), 15)  #curvefitting only yello cone
				#-----------------------------------------------------------------------------------
				for j in range(18):
					x,y = curve_fitting_2point(int(warp_left_point[0]), int(yellow_arr_curve[0][1]), int(warp_left_point[1]), int(yellow_arr_curve[0][2]), 20)
					cv2.line(img_transformed_y, (int(x[j]),int(y[j])), (int(x[j+1]),int(y[j+1])),(255,0,0),15)
					#cv2.line(img_transformed_y, (int(yellow_arr[j][1]), int(yellow_arr[j][2])), (int(yellow_arr[j+1][1]), int(yellow_arr[j+1][2])), (255,0,0), 15)
					cv2.line(yellow_img, (int(x[j]),int(y[j])), (int(x[j+1]),int(y[j+1])),(255,255,255),15)
					#cv2.line(yellow_img, (int(yellow_arr[j][1]), int(yellow_arr[j][2])), (int(yellow_arr[j+1][1]), int(yellow_arr[j+1][2])), (255,255,255), 15)'''
			if (len(blue_arr) >= 3):
				#modify-----------------------------------------------------------------------------
				x,y = curve_fitting(blue_arr,20) 
				for k in range((len(x)-2), 0 , -1): 
					cv2.line(blue_img, (int(x[k]),int(y[k])), (int(x[k+1]),int(y[k+1])), (255,255,255), 15)  #curvefitting only blue cone
					cv2.line(img_transformed_b, (int(x[k]),int(y[k])), (int(x[k+1]),int(y[k+1])), (0,255,0), 15)  #curvefitting only blue cone
				#-----------------------------------------------------------------------------------
				'''
				for j in range(18):
					x,y = curve_fitting_2point(int(warp_right_point[0]), int(blue_arr_curve[0][1]), int(warp_right_point[1]), int(blue_arr_curve[0][2]), 20)
					cv2.line(img_transformed_b,  (int(x[j]),int(y[j])), (int(x[j+1]),int(y[j+1])), (0,255,0), 15)
					#cv2.line(img_transformed_b, (int(blue_arr[j][1]), int(blue_arr[j][2])), (int(blue_arr[j+1][1]), int(blue_arr[j+1][2])), (0,255,0), 15)
					cv2.line(blue_img,  (int(x[j]),int(y[j])), (int(x[j+1]),int(y[j+1])), (255,255,255), 15)
