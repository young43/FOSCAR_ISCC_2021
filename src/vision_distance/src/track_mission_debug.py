...
def intrp_fitting(pts, num_of_pts):
	print("*********************************")
	pts = np.array(pts)
	print(pts)
	new_pts_x = []
	new_pts_y = []
	for pt in pts:
		new_pts_x.append(pt[1])
		new_pts_y.append(pt[2])
	
	#new_pts = np.array(new_pts)
	#print()
	#print(np.min(new_pts[:][0]))
	#print(np.max(new_pts[:][0]))
	#x = np.linspace(np.min(new_pts[:][0]), np.max(new_pts[:][0]), num_of_pts)
	#print(x)
	#print(new_pts[:][0])
	try:
		test = interp1d(new_pts_y, new_pts_x, kind = 'linear')
	except: return

	x_set = np.linspace(min(new_pts_y), max(new_pts_x), num_of_pts)
	y_set = test(x_set)
	print(x_set)
	print(y_set) 
	print("*********************************")
