...
# margin, self.car_angle_deg_2+=5, min_list[0]<1.0 and abs(min_list[1].y)<0.8, in_step=0.1, de_step=0.2, limit_speed=7, bottom_speed=3
...
class point:
	...
	def calc_angle(self):
		self.car_angle = math.atan2(self.center_y,self.center_x)
		self.car_angle_deg = self.car_angle*180/math.pi
		self.car_angle_deg_2 = self.car_angle*180/math.pi
		...
		if len(self.obData.circles) == 0:
			...
			sorted_list_3=[1,2,3]
 			...
 			if(len(circles)==2):
				...
				if (len(yellow_cone)>=1) and len(blue_cone)==0 :
					drive(30,True)
				elif (len(blue_cone)>=1) and len(yellow_cone)==0 :
					drive(-30,True)
				else:	
...
if __name__=='__main__':
	...
	print('Done')
 315  src/obstacle_detector/src/track_drive2.py 
@@ -0,0 +1,315 @@
#!/usr/bin/env python
